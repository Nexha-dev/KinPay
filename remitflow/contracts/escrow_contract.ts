export type ReleasePlan = {
  ...
  intervalMs: number;   // for periodic release
  portions: number;     // number of equal releases
...
export class EscrowContract {
  ...
  releaseDue(id: string, now = Date.now()) {
    ...
    if (plan.emergencyUnlocked) {
      ...
      return { amount: remaining, status: 'emergency', plan };
    }

    const elapsed = Math.max(0, now - plan.startAt);
    const steps = Math.floor(elapsed / plan.intervalMs);
    const totalDue = Math.min(plan.totalAmount, Math.floor((plan.totalAmount / plan.portions) * steps));
    const toRelease = Math.max(0, totalDue - plan.releasedAmount);

    plan.releasedAmount += toRelease;
    return { amount: toRelease, status: 'scheduled', releasedAmount: plan.releasedAmount, plan };
  }
